...
train_datagen = ImageDataGenerator(
    ...
    shear_range=0.1,                # set the shear intensity
    zoom_range=0.1,                 # random zoom
    horizontal_flip=True,           # randomly flip images horizontally
    brightness_range=[0.5, 1.5],    # random brightness adjustment
    fill_mode='nearest',            # filling points outside the input boundaries
...
model_top = Sequential([
    Conv2D(8, (1, 1), use_bias=False, kernel_initializer='uniform', activation='relu', input_shape=base_model.output_shape[1:]),
    MaxPooling2D(1, 1),
    Flatten(),
